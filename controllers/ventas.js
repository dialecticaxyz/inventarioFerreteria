...
function deleteOnlyVenta(req, resp){
  ventas.find({"_id": req.body["id"]}, (err, vent)=>{
    if(vent.length==0){
      resp.send("empty")
    }else{
      ventas.remove({_id: req.body["id"]},{}, (err, numRemoved)=>{
        if(numRemoved==1){ resp.send("delet") }else{ resp.send("deletFail") }
      });
    }
  });
